...
python3 "${APP_DIR}/scripts/generate_baseline_club_knowledge.py"
python3 "${APP_DIR}/scripts/generate_demo_candidate_documents.py"
...
grep '^BASELINE_SET_ID ' "${LOG_FILE}" || true

PERSIST_SESSION=$(grep '^SESSION_MAIN ' "${LOG_FILE}" | tail -1 | awk '{print $2}' || true)
PERSIST_SESSION="${PERSIST_SESSION:-$(grep '^SESSION_ISO ' "${LOG_FILE}" | tail -1 | awk '{print $2}' || true)}"

echo "=== PERSISTENCE RESTART ==="
sudo docker restart "${CANDIDATE}" >/dev/null
sleep 12
if [ -n "${PERSIST_SESSION}" ]; then
  HTTP_CODE=$(curl -sS -o /tmp/baseline_gate_persist.json -w '%{http_code}' "${BASE}/api/sessions/${PERSIST_SESSION}" || echo "000")
...
sudo docker exec "${CANDIDATE}" python scripts/reconcile_session_documents.py --dry-run | tee /tmp/baseline_gate_reconcile.log
